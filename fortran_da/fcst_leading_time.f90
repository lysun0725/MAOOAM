PROGRAM fcst_leading_time
  ...
  ALLOCATE(X_ocn(2*noc),X_lead_ocn(2*noc))
  ...
  ALLOCATE(R(2*natm),luse(2*natm)); luse = .true.
...
! set initial ensembles
  ...
  DO WHILE (t<t_run_da)
    ...
    IF (mod(t,tw_da)<dt) THEN
      ...
      DO k = 1, ens_num
        err_lead_sum(1,:) = err_lead_sum(1,:) + (X_lead(1:2*natm,k)-X_lead_true(1:2*natm))
      ENDDO
      ...
      DO WHILE (t_tmp_lead<t_lead)
        ...
        IF (mod(t_tmp_lead,tw)<dt) THEN
          ...
          IF (mod(t_tmp_lead,tw_solo)<dt) THEN
            X_lead_ocn = X_lead_true(2*natm+1:ndim)
          END IF
        END IF
      ENDDO
    ELSE
