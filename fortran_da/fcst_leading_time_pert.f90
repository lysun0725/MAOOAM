PROGRAM fcst_leading_time_pert
  ...
  REAL(KIND=8),DIMENSION(:),ALLOCATABLE :: X_lead_ocn
  ...
  REAL(KIND=8),DIMENSION(:),ALLOCATABLE :: R, R_ocn
  ...
  ALLOCATE(X_ocn(2*noc),X_lead_ocn(2*noc),err_lead(2*noc))
  ALLOCATE(X_lead_true(0:ndim))
  ALLOCATE(X_lead(2*natm,ens_num))
  ALLOCATE(err_lead_ave(nt_lead,2*natm),err_lead_sum(nt_lead,2*natm)); err_lead_sum=0.d0
  ALLOCATE(Xens(2*natm,ens_num),Xens_a(2*natm,ens_num))
  ALLOCATE(R(2*natm),luse(2*natm),R_ocn(2*noc)); luse = .true.
...
! load obs
  ...
  DO j = 1, ndim
    IF (j .ge. 2*natm+1) THEN
      READ(103,"(10000(D24.17,1x))") R_ocn(j-2*natm)
    ELSE
...
! set initial ensembles
  ...
  DO k = 1,ens_num
    ...
    Xens(:,k) = X(1:2*natm) + (err*R)*ini_err
  ENDDO
  ...
  DO WHILE (t<t_run_da)
    ...
    IF (mod(t,tw_da)<dt) THEN
      ...
      cnt_da = cnt_da + 1
      X_lead = Xens_a
      X_lead_true(0:ndim) = Xhist(cnt_obs+1,0:ndim); X_lead_ocn = X_lead_true(2*natm+1:ndim)

      DO k = 1, ens_num
        err_lead_sum(1,:) = err_lead_sum(1,:) + ABS(X_lead(1:2*natm,k)-X_lead_true(1:2*natm))
      ENDDO
      ...
      DO WHILE (t_tmp_lead<t_lead)
        DO k = 1, ens_num
          ...
          X_lead_solo(0:ndim) = (/1.D0,X_lead(:,k),X_lead_ocn/)
          CALL step(X_lead_solo,t_tmp_lead2,dt,X_lead_solo_new,soloflag)
          X_lead(:,k)=X_lead_solo_new(1:2*natm)
        ENDDO
        ...
        IF (mod(t_tmp_lead,tw)<dt) THEN
          ...
          DO k = 1, ens_num
            err_lead_sum(cnt_lead,:) = err_lead_sum(cnt_lead,:) + ABS(X_lead(1:2*natm,k)-X_lead_true(1:2*natm))
          ENDDO
          err_lead_ave(cnt_lead,:) = err_lead_sum(cnt_lead,:)/cnt_da/ens_num

          IF (mod(t_tmp_lead,tw_solo)<dt) THEN
            CALL randn(2*noc,err_lead)
            X_lead_ocn = X_lead_true(1:2*noc) + err_lead*R_ocn
          END IF
        END IF
      ENDDO
    ELSE
    ...
    ! Write ensemble spread
      ...
      IF (mod(t,tw_solo)<dt) THEN
        CALL randn(2*noc,err_lead)
        X_ocn = X(2*natm+1:ndim)+err_lead*R_ocn
      END IF
