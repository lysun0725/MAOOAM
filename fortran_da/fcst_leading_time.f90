PROGRAM fcst_leading_time
  ...
  ALLOCATE(X_atm(2*natm),X_lead_atm(2*natm))
  ...
  ALLOCATE(R(2*noc),luse(2*noc)); luse = .true.
...
! load obs
  ...
  DO j = 1, ndim
    ...
    R(j-2*natm) = R(j-2*natm) ! Try 10% climatology 
  ENDDO
  CLOSE(103)
...
! set initial ensembles
  ...
  DO k = 1,ens_num
    ...
    Xens(:,k) = X(2*natm+1:ndim) + (err*R)*ini_err*0.1
  ENDDO
      ...
      DO WHILE (t_tmp_lead<t_lead)
        ...
        IF (mod(t_tmp_lead,tw)<dt) THEN
          ...
          IF (mod(t_tmp_lead,tw_solo)<dt) THEN
            X_lead_atm = X_lead_true(1:2*natm)
          END IF
        END IF
      ENDDO
    ELSE
