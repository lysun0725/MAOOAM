PROGRAM freerun_maooam
  ...
  IF (writeout) THEN
      IF (tw_solo .lt. 1) THEN
          write(soloname,'("freerun_ocn",F0.3,".dat")') tw_solo
      ELSE
          write(soloname,'("freerun_ocn",I4.4,".dat")') INT(tw_solo)
      ENDIF
  ...
  ! Add noise to X_ocn and initialize X_solo and X_atm
  CALL randn(ndim,err)
  DO n = 1,2*noc
     X_ocn(n) = X(n+2*natm) + err(n+2*natm)*R(n+2*natm)
  END DO
  X_atm = X(1:2*natm)
  ...
  DO WHILE (t<t_run)
     ...
     CALL step(X_solo, t_solo, dt, X_solo_new,'ocn')
     ...
     IF (mod(t,tw_solo)<dt) THEN
        X_atm = X(1:2*natm)
        X_solo(1:2*natm) = X_atm ! Update the ocn part in X_sol
     END IF
